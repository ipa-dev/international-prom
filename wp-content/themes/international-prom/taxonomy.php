			...
			<div class="col span_4_of_12" style="text-align: right;">
				...
				<form style="display: inline-block; width: auto;" method="POST" action="">
					<select style="display: inline-block; width: auto;" style="" name="order_select" onchange="this.form.submit()">
						<option
						<?php
						if ( $_SESSION['order_select'] == 'name_asc' ) {
							echo ' selected="selected"'; }
						?>
						 value="name_asc">Title</option>
						<option
						<?php
						if ( $_SESSION['order_select'] == 'date_asc' ) {
							echo ' selected="selected"'; }
						?>
						 value="date_asc">Oldest</option>
						<option
						<?php
						if ( $_SESSION['order_select'] == 'date_desc' ) {
							echo ' selected="selected"'; }
						?>
						 value="date_desc">Newest</option>
						<option
						<?php
						if ( $_SESSION['order_select'] == 'style_no' ) {
							echo ' selected="selected"'; }
						?>
						 value="style_no">Style Number</option>
					</select>
			...
			<div class="col span_2_of_12" style="text-align: right;">
				...
				<form style="display: inline-block; width: auto;" method="POST" action="">
					<select style="display: inline-block; width: auto;" style="" name="num_select" onchange="this.form.submit()">
						<option
						<?php
						if ( $_SESSION['num_select'] == 10 ) {
							echo ' selected="selected"'; }
						?>
						 value="10">10</option>
						<option
						<?php
						if ( $_SESSION['num_select'] == 20 ) {
							echo ' selected="selected"'; }
						?>
						 value="20">20</option>
						<option
						<?php
						if ( $_SESSION['num_select'] == 30 ) {
							echo ' selected="selected"'; }
						?>
						 value="30">30</option>
						<option
						<?php
						if ( $_SESSION['num_select'] == 50 ) {
							echo ' selected="selected"'; }
						?>
						 value="50">50</option>
						<option
						<?php
						if ( $_SESSION['num_select'] == 100 ) {
							echo ' selected="selected"'; }
						?>
						...
						<div class="fav">
							...
							<a href="javascript:void(0)" id="fav-<?php echo get_the_ID(); ?>" 
								...
								onclick="removeFav<?php echo get_the_ID(); ?>();" title="[-] Remove from favorites"
																				...
																				?>
								onclick="addFav<?php echo get_the_ID(); ?>();" title="[+] Add as favorite"<?php } ?>><i class="fa fa-heart"></i></a>
							<?php } else { ?>
