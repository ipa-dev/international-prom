function multiUploader(config){
	...
	this.config = config;
    var refreshUrl1 = this.config.refreshUrl;
		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="'+Url+'/images/image.png" />';
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
						sampleIcon = '<img src="'+Url+'/images/unknown.png" />';
						errorClass =" invalid";
					} 
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress1" style="display:none;"><img src="'+Url+'/images/ajax-loader.gif" /></div></div>';
				}
			}
	...
	multiUploader.prototype._uploader = function(file,f){
		if(typeof file[f] != undefined && self._validate(file[f].type) > 0){
			...
			jQuery(".dfiles[rel='"+ids+"']").find(".progress1").show();
			jQuery.ajax({
				type:"POST",
				...
				success:function(rponse){
					jQuery("#"+ids).hide();
                    jQuery.ajax(refreshUrl1, {
                       type: "POST",
                       success: function (data) {
                           jQuery(".imageContent").html(data);
                       } 
                    });
