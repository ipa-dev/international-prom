...
abstract class AbstractRemoteAccount
{
    ...
    public function fetch()
    {
        if (!is_null($this->localID)) {
            //This return an empty string if not set
            $attributes = get_post_meta($this->localID, 'remote_attr', true);
            
            //attributes property need be an array to avoid PHP 7.1 error, so only set if attributes is not empty
            if (!empty($attributes)) {
    ...
    public function hasOption($name, $value)
    {
        if (isset($this->attributes[$this->type.'_'.$name]) &&
            $this->attributes[$this->type.'_'.$name] === $value
        ) {
            return true;
        }
    ...
    public function isAuthNeeded()
    {
        if ($this->service instanceof AbstractAuthService) {
            return $this->service->isAuthNeeded();
        }
