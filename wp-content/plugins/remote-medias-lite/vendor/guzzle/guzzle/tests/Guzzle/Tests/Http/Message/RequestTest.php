...
namespace WPRemoteMediaExt\Guzzle\Tests\Http\Message;

use WPRemoteMediaExt\Guzzle\Common\Collection;
use WPRemoteMediaExt\Guzzle\Http\EntityBody;
use WPRemoteMediaExt\Guzzle\Http\Url;
...
use WPRemoteMediaExt\Guzzle\Http\RedirectPlugin;
use WPRemoteMediaExt\Guzzle\Http\Exception\BadResponseException;

/**
 * @group server
 * @covers WPRemoteMediaExt\Guzzle\Http\Message\Request
 * @covers WPRemoteMediaExt\Guzzle\Http\Message\AbstractMessage
 */
class RequestTest extends \WPRemoteMediaExt\Guzzle\Tests\GuzzleTestCase
{
    /** @var Request */
    protected $request;
    ...
    public function testSend()
    {
        ...
        $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\\Http\\Message\\Response', $this->request->getResponse());
    ...
    public function testGetResponse()
    {
        ...
        $requestResponse = $request->getResponse();
        $this->assertSame($response, $requestResponse);
        $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\\Http\\EntityBody', $response->getBody());
    }
    ...
    public function testRequestThrowsExceptionOnBadResponse()
    {
        try {
            ...
            $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\\Http\\Message\\RequestInterface', $e->getRequest());
            $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\\Http\\Message\\Response', $e->getResponse());
    ...
    {
        $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\\Http\\QueryString', $this->request->getQuery());
    ...
    /**
     * @expectedException \WPRemoteMediaExt\Guzzle\Http\Exception\RequestException
    ...
    public function testCanRetrieveUrlObject()
    {
        ...
        $this->assertInstanceOf('WPRemoteMediaExt\Guzzle\Http\Url', $request->getUrl(true));
